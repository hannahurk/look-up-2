...
A ceiling sign that cycles through five full-screen views of live NASA data: today's Astronomy Picture of the Day, a space-weather readout, a live Earth image, Mars weather, and "Algorithm Art" — a generative canvas piece that translates the same live space-weather and near-Earth-object data into slow, ambient motion (not a dashboard, not a literal solar-system diagram).

## What's driving it

A Vercel serverless function (`api/nasa-data.js`) is the only thing that holds the real NASA API key (`process.env.NASA_API_KEY`, never sent to the browser). It requests six endpoints in parallel with `Promise.allSettled` — so one failure never blocks the rest — and returns a small normalized payload:
...
- [`insight_weather`](https://api.nasa.gov) — InSight's last available Mars weather reading
...
Each time the sign wakes from idle (not on every twitch while already awake), it advances: **APOD photo → Cosmic Meteorology → EPIC Earth image → Mars weather → Algorithm Art → back to APOD.**
...
- **Mars weather** — InSight's last available reading. Its mission ended in December 2022, and this feed has been frozen even longer than that, so the screen says plainly when the reading is from rather than presenting stale data as current.
...
- `style.css` — full-viewport layout, the shared weather/Mars "stat screen" style, the canvas/grain styling, and the mode-switching + idle/wake transitions
- `app.js` — fetches and renders APOD, EPIC, solar wind, Cosmic Meteorology, and Mars weather; runs the Algorithm Art generative engine; and drives the five-way idle/wake cycle
- `api/nasa-data.js` — the Vercel serverless function that fetches and normalizes APOD, NEO, DONKI, and Mars data
